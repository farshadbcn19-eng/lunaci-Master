...
// Exact code re-embedded from the live snippet, confirmed via
// diagnose-snippet8-full-dump.yml run immediately before this workflow was
// authored (length 7291 - this is the post-fix-home-nav-seam.yml state,
// where the .ln-nav gradient's end stop was already floored at
// rgba(11,11,11,.55) instead of transparent).
...
.ln-nav{position:fixed;top:0;left:0;right:0;z-index:9999;display:flex;align-items:center;justify-content:space-between;padding:16px 5%;background:linear-gradient(to bottom,rgba(11,11,11,.95),rgba(11,11,11,.55));transition:background .4s;border-bottom:1px solid transparent;}
...
// The client reports the header background between the logo and the icons
// still reads lighter than the logo's own solid black backdrop, even after
// flooring the gradient at alpha .55 (fix-home-nav-seam.php). Make the whole
// Home-page header solid instead of a gradient, matching the same
// rgba(11,11,11,.97) already used for the scrolled state and for all
// non-Home pages, so the header reads as one continuous dark bar at parity
// with the logo image's own background - no more mismatch, on any page.
$old_bg = 'background:linear-gradient(to bottom,rgba(11,11,11,.95),rgba(11,11,11,.55))';
$new_bg = 'background:rgba(11,11,11,.97)';
...
$old_count = substr_count( $row['code'], $old_bg );
echo "occurrences of target background rule in stored code: $old_count\n";
if ( 1 !== $old_count ) {
	echo "ERROR: expected exactly 1 occurrence of the target background rule, found $old_count - refusing to modify (ambiguous target)\n";
	echo "ABORT\n";
	exit( 1 );
}
...
echo "\n=====================================================\n";
echo "STEP B: COMMIT - make the Home-page nav background solid, matching non-Home/scrolled state\n";
echo "=====================================================\n";

$updated_code = str_replace( $old_bg, $new_bg, $row['code'], $replace_count );
...
if ( ! $verify_row ) {
	...
	$has_new_rule = ( false !== strpos( $verify_row['code'], $new_bg ) );
	echo "new solid background rule present in stored code: " . ( $has_new_rule ? 'YES' : 'NO' ) . "\n";

	$old_gone = ( false === strpos( $verify_row['code'], $old_bg ) );
	echo "old gradient rule fully removed: " . ( $old_gone ? 'YES' : 'NO' ) . "\n";
...
echo "OK: Home-page nav background made solid rgba(11,11,11,.97) on snippet id=$snippet_id, active=1, rest of code unchanged.\n";
