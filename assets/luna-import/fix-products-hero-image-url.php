<?php
/**
 * Guarded fix: the user re-uploaded a replacement banner image directly
 * via the WordPress media library (not through this repo's import
 * workflow). WordPress auto-renamed it to avoid a filename collision, so
 * its URL is now lunaimport-product-hero-luna-1.jpg instead of
 * lunaimport-product-hero-luna.jpg. Update the .page-hero::before
 * background-image URL in the WPCode CSS snippet (post 483) to point at
 * the new filename, bumping the cache-busting query string. No other
 * text/CSS is touched.
 *
...
$old_url = "url('https://lunacibarcelona.com/wp-content/uploads/2026/08/lunaimport-product-hero-luna.jpg?v=2') center/cover no-repeat;";
$new_url = "url('https://lunacibarcelona.com/wp-content/uploads/2026/08/lunaimport-product-hero-luna-1.jpg?v=3') center/cover no-repeat;";
...
if ( 1 !== $old_count ) {
	echo "ERROR: expected exactly 1 occurrence of the old image URL rule, found {$old_count}\n";
	echo "ABORT: refusing to write against unexpected/changed content\n";
	exit( 1 );
}
echo "OK: found exactly 1 occurrence of the old image URL rule\n";

if ( false !== strpos( $current, 'lunaimport-product-hero-luna-1.jpg' ) ) {
	echo "ERROR: the new image filename already appears in the content - refusing to duplicate/re-run\n";
	echo "ABORT\n";
	exit( 1 );
}
echo "OK: new image filename not already present\n";
...
if ( false === strpos( $verify_content, 'lunaimport-product-hero-luna-1.jpg?v=3' ) ) {
...
if ( false !== strpos( $verify_content, 'lunaimport-product-hero-luna.jpg?v=2' ) ) {
