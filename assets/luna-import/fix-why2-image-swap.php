...
$page_id                  = 57;
$expected_byte_len        = 28512; // confirmed live _elementor_data byte length (read-only verification, 2026-08-23 ~16:56 UTC)
$target_container_id      = 'bf4109a';
$target_widget_id         = '9b0a463';
$new_content_path         = '/tmp/lunaimport-page57-widget-why2.html';
$expected_new_content_len = 26934; // byte length of the transferred replacement file

echo "####################################################################\n";
echo "Fix: swap Why-Lunaci (In 15 Seconds) image src, page ID={$page_id}, container[id={$target_container_id}]/widget[id={$target_widget_id}]\n";
echo "####################################################################\n\n";
...
$required_substrings = array(
	'lunaimport-why2-luna-replacement.jpg?v=2',
	'class="ln-origin"', // sanity: confirms this build still has the origin section from the prior change
);
...
if ( false !== strpos( $new_content, 'lunaci-category-Lips-1.png' ) ) {
	echo "ABORT: old why2 image src still present in new content - replacement did not remove it.\n";
	return;
}
echo "PASS: new why2 image URL present, old image URL absent, Our Origin section still present.\n\n";
...
if ( null === $decoded_after ) {
	echo "FAIL: could not decode _elementor_data after write.\n";
...
// Also clear Elementor's own CSS/render cache for this page - lesson learned from
// the earlier Our Origin deployment, where a stale _elementor_css cache kept
// serving the old page even after the DB write and full page-cache purge.
if ( class_exists( '\\Elementor\\Plugin' ) ) {
	try {
		\Elementor\Plugin::instance()->files_manager->clear_cache();
		echo "\nElementor files_manager->clear_cache(): done\n";
	} catch ( \Throwable $e ) {
		echo "\nElementor files_manager->clear_cache() failed: " . $e->getMessage() . "\n";
	}
	delete_post_meta( $page_id, '_elementor_css' );
	echo "Deleted _elementor_css postmeta for page {$page_id}: done\n";
}
wp_cache_flush();
echo "wp_cache_flush(): done\n";

echo "\nOK: fix-why2-image-swap completed\n";
